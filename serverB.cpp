...
void booting_up_B() {
	cout << "The Server B is up and running." << endl;
...
//Read serverB.txt to get serverB's neighbor information
void readfileB() {
    string str;
    int lines_index = 0;
	ifstream readfile("serverB.txt", ios::in); //load file
	if (!readfile){ //check if the file is loaded
		cout << "Cannot Open File" << endl;
		return;
	}
	cout << "The Server B has the following neighbor information :" << '\n' << "Neighbor---Cost" << endl;
	send_str.append("B/");
    ...
    while (getline(readfile, str))
	{
		...
		if (str1 == "serverA") {
			...
			adjacency_matrix[3][1] = cost;
			adjacency_matrix[1][3] = cost;
            send_str.append("D/");
		}
        stringstream ss;
        ss << cost;
...
void setupTCP(){
    ...
    cout << "The Server B finishes sending its neighbor information to the Client with TCP port number " << CLIENT_TCP << " and IP address " << localIP << endl;
    cout << "For this connection with the Client, the Server B has TCP port number "<< portNumTCP << " and IP address "<< ip << endl;
    cout << '\n';
    close(server);
}
...
void setupUDP(){
    ...
    memset((char *) &server_addr, 0, sizeof(server_addr));
    server_addr.sin_family = AF_INET;
    server_addr.sin_port = htons(SERVER_B_UDP);
    ...
    cout << "The server B has received the network topology from the Client with UDP port number "<< port_num << " and IP address "<< ip <<" as follows:" << endl;
    string str(buffer);
    getFromServer(str);
    close(server);
    cout << "For this connection with Client, The Server B has UDP portnumber "<< SERVER_B_UDP << " and IP address " << localIP << endl;
}

int main(){
	/*Create a Socket for incoming connections
	port # = 22000+638 = 22638*/
	booting_up_B();
	readfileB();
	setupTCP();
    setupUDP();

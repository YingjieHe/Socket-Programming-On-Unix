...
//Read serverD.txt to get serverD's neighbor information
void readfileD() {
    string str;
    int lines_index = 0;
	ifstream readfile("serverD.txt", ios::in); //load file
	if (!readfile){ //check if the file is loaded
		cout << "Cannot Open File" << endl;
		return;
	}
	cout << "The Server D has the following neighbor information :" << '\n' << "Neighbor---Cost" << endl;
	send_str.append("D/");
...
void setupTCP(){
    ...
    cout << "The Server D finishes sending its neighbor information to the Client with TCP port number " << CLIENT_TCP << " and IP address " << localIP << endl;
...
void setupUDP(){
    ...
    memset((char *) &server_addr, 0, sizeof(server_addr));
    server_addr.sin_family = AF_INET;
    server_addr.sin_port = htons(SERVER_D_UDP);
    ...
    cout << "The server D has received the network topology from the Client with UDP port number "<< port_num << " and IP address "<< ip <<" as follows:" << endl;
    string str(buffer);
    getFromServer(str);
    close(server);
    cout << "For this connection with Client, The Server D has UDP portnumber " << SERVER_D_UDP <<" and IP address " << localIP << endl;
}

int main(){
	/*Create a Socket for incoming connections
	port # = 24000+638 = 24638*/
	booting_up_D();
	readfileD();
	setupTCP();
    setupUDP();

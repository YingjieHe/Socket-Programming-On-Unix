...
void booting_up_C() {
	cout << "The Server C is up and running." << endl;
...
//Read serverC.txt to get serverC's neighbor information
void readfileC() {
    string str;
    int lines_index = 0;
	ifstream readfile("serverC.txt", ios::in); //load file
	if (!readfile){ //check if the file is loaded
		cout << "Cannot Open File" << endl;
		return;
	}
	cout << "The Server C has the following neighbor information :" << '\n' << "Neighbor---Cost" << endl;
	send_str.append("C/");
    ...
    while (getline(readfile, str))
	{
		...
		if (str1 == "serverB") {
			adjacency_matrix[1][2] = cost;
			adjacency_matrix[2][1] = cost;
            send_str.append("B/");
		}
		else if (str1 == "serverA") {
			adjacency_matrix[2][0] = cost;
			adjacency_matrix[0][2] = cost;
            send_str.append("A/");
		}
		else if (str1 == "serverD") {
			adjacency_matrix[3][2] = cost;
			adjacency_matrix[2][3] = cost;
            send_str.append("D/");
		}
        stringstream ss;
        ss << cost;
...
void setupTCP(){
    ...
    cout << "The Server C finishes sending its neighbor information to the Client with TCP port number " << CLIENT_TCP << " and IP address " << localIP << endl;
    cout << "For this connection with the Client, the Server C has TCP port number "<< portNumTCP << " and IP address "<< ip << endl;
    cout << '\n';
    close(server);
}
...
void setupUDP(){
    ...
    memset((char *) &server_addr, 0, sizeof(server_addr));
    server_addr.sin_family = AF_INET;
    server_addr.sin_port = htons(SERVER_C_UDP);
    ...
    cout << "The server C has received the network topology from the Client with UDP port number "<< port_num << " and IP address "<< ip <<" as follows:" << endl;
    string str(buffer);
    getFromServer(str);
    close(server);
    cout << "For this connection with Client, The Server C has UDP portnumber " << SERVER_C_UDP  << " and IP address " << localIP << endl;
}

int main(){
	/*Create a Socket for incoming connections
	port # = 23000+638 = 23638*/
	booting_up_C();
	readfileC();
	setupTCP();
    setupUDP();
